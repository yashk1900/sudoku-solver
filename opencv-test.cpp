...
#include <iostream>
#include "C:\Users\shash\source\repos\opencv-test\digitrecognizer.h"
#include "D:\yash\OpenCV\opencv\build\include\opencv2\stdafx.h"
...
void recursive(int x, int y)
{
	...
	if (x == 9 && y == 9)
	{
		...
		else
		{
			rep(i, 1, 10)
			{
				if (r[x][i] == 0 && c[y][i] == 0 && adj[mp[{x, y}]][i] == 0)
				{
					q[x][y] = i;
					rep(i1, 1, 10)
					{
...
int main()
{
	Mat sudoku = imread("D:\yash\OpenCV\opencv\image.jpg", 0);
	...
	char* m21 = (char*)"D:\character_recognition\train-images.idx3-ubyte"; char* m31 = (char*)"D:\character_recognition\train-labels.idx1-ubyte";
	bool b = dr->train(m21, m31);
	printf("Trained: %d\n", b);
